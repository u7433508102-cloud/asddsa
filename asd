        ...
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            if not isPlayerKnockedOrKO(player) then
                ...
                if part and canSeeTarget(part) then
                    ...
                    if dist < bestDist then bestDist = dist; bestPlayer = player end
                end
            end
        end
    end
...
local function applyCameraLock()
    ...
    if isSelfKnocked() then camlockTarget = nil; return end
    if camlockTarget then
        local char = camlockTarget.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        if not hrp or not hum or hum.Health <= 0 or isPlayerKnockedOrKO(camlockTarget) then camlockTarget = nil end
    end
    if not camlockTarget then camlockTarget = findCamlockTarget() end
    ...
    local alpha = 1 / Config['Camera Lock']['Smoothing']
...
if not fovBox then
    ...
    fovBox.Visible = false; fovBox.Thickness = Config['FOV']['Thickness']
...
getrawmetatable(game).__index = function(self, key)
    ...
    if not player or isPlayerKnockedOrKO(player) or not canSeeTarget(currentTarget) then return oldIndex(self, key) end
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if input.KeyCode == Enum.KeyCode[Config['Keybinds']['Camera Lock']] then
        camlockEnabled = not camlockEnabled; camlockTarget = nil
    end
end)
...
-- ========== GUI (PIXEL‑COMPACT, NO RAPID FIRE) ==========
